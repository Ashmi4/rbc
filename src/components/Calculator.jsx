...
function Calculator() {
  ...
  const handleInputChange = (event) =>
    setuserAmount({ ...userAmount, [event.target.name]: event.target.value });

    // Manage validations and error messages
  const isValid = () => {
    const { amount, interest, years } = userAmount;
    let actualError = '';
      ...
      <div className='form'>
        ...
        <form onSubmit={handleSubmitValues}>
          ...
          {!results.isResult ? (
            ...
            <div className='form-items'>
              <div>
                ...
                <input
                  ...
                  // onChange method sets the values given by the user as input to the userAmount state
              ...
              <div>
                ...
                <input
                  ...
                  value={userAmount.interest}
                  onChange={handleInputChange}
                />
              </div>
              <div>
                <label id='label'>Years Pay:</label>
                <input
                  type='text'
                  name='years'
                  placeholder='Years to repay'
                  value={userAmount.years}
                  onChange={handleInputChange}
                />
              </div>
            ...
            <div className='form-items'>
              <h4>
                Loan amount: ${userAmount.amount} <br /> Interest:{' '}
                {userAmount.interest}% <br /> Years to repay: {userAmount.years}
              </h4>
              <h4>
              Monthly Payment: ${results.monthlyPayment} <br /> Total Interest:{' '}
                {results.totalInterest}% <br /> Total Payment: {results.totalPayment}
              </h4>              
